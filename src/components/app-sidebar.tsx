  ...
  return (
    <aside className="flex h-full w-[268px] shrink-0 flex-col border-r border-sidebar-border bg-sidebar">
      ...
      <div className="flex items-center gap-2.5 px-4 pb-3 pt-5">
        ...
        <img src="/petrol-ofisi.svg" alt="Petrol Ofisi" className="h-6 w-auto" />
        <span className="mt-0.5 rounded border border-sidebar-accent px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-sidebar-foreground/60">
          {appName}
        </span>
      </div>
      ...
      <div className="px-3 pb-1 pt-2">
        <Button className="w-full justify-start gap-2.5" onClick={() => router.push("/chat")}>
      ...
      <nav className="flex flex-col gap-0.5 px-3 py-2">
        ...
        {user.role === "ADMIN" && (
          <div className="mt-2">
            <div className="px-2.5 pb-1 text-[11px] font-semibold uppercase tracking-wider text-sidebar-foreground/50">
              Admin
            </div>
      ...
      <div className="mt-1 flex-1 overflow-y-auto px-3 pb-2">
        {projects.length > 0 && (
          <>
            <div className="px-2.5 pb-1 pt-2 text-[11px] font-semibold uppercase tracking-wider text-sidebar-foreground/50">
              Projects
            </div>
        ...
        <div className="px-2.5 pb-1 pt-3 text-[11px] font-semibold uppercase tracking-wider text-sidebar-foreground/50">
          Recents
        </div>
        <div className="flex flex-col gap-0.5">
          {chats.length === 0 && (
            <p className="px-2.5 py-1.5 text-sm text-sidebar-foreground/50">No chats yet</p>
          )}
          {chats.map((chat) => {
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  className="hidden shrink-0 rounded p-0.5 text-sidebar-foreground/50 hover:text-primary group-hover:block cursor-pointer"
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="flex w-full items-center gap-2.5 rounded-lg px-2 py-1.5 text-left transition-colors hover:bg-sidebar-accent/60 cursor-pointer">
              <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-white/10 text-xs font-semibold uppercase text-sidebar-foreground">
                {user.name.slice(0, 1)}
              </div>
