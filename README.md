...
- 📄 **Attach files to any chat** (PDF, Word, Excel, PowerPoint, text/code): contents are extracted
  server-side and stay available to the model for the whole conversation
- 🖼️ **Attach images** (JPEG/PNG/WebP): compressed client-side and sent to vision-capable models as
  real image input (Claude, GPT, Gemini)
...
- 🗂 **Projects** — group chats with shared **Instructions**, **Memory**, and a **Knowledge** base
  (upload PDF / DOCX / XLSX / PPTX / TXT / MD / CSV / code files; chats automatically retrieve relevant excerpts)
...
- **RAG ingestion**: upload → text extraction (`pdf-parse`, `mammoth`, `exceljs`, PPTX slide/notes
  text via `jszip`) → paragraph-aware chunking
